...
### `UpdateActivityOptions` - NOT IMPLEMENTED

API allows updating activity options (retry policy, timeouts, etc.) on running activities.
...
message UpdateActivityOptionsRequest {
    ...
    oneof activity { string id = 6; string type = 7; bool match_all = 9; }
    bool restore_original = 8;
}
```

**Note:** Design docs say this is planned for "MLP GA" (not pre-release).

### `PauseActivity` / `UnpauseActivity` - NOT IMPLEMENTED

API allows pausing and unpausing activities.
...
message PauseActivityRequest {
    ...
    temporal.api.common.v1.WorkflowExecution execution = 2;
    string identity = 3;
    oneof activity { string id = 4; string type = 5; }
    string reason = 6;
}

message UnpauseActivityRequest {
    // ... plus reset_attempts, reset_heartbeat, jitter options
}
```

**Note:** Design docs say this is planned for "MLP GA" (not pre-release).

### `ResetActivity` - NOT IMPLEMENTED

API allows resetting activities to initial state.
...
message ResetActivityRequest {
    ...
    temporal.api.common.v1.WorkflowExecution execution = 2;
    string identity = 3;
    oneof activity { string id = 4; string type = 5; bool match_all = 10; }
    bool reset_heartbeat = 6;
    bool keep_paused = 7;
    google.protobuf.Duration jitter = 8;
    bool restore_original = 9;
}
```
...
API allows deleting activity execution (cleanup).
