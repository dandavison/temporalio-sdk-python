...
def test_command_aware_visitor_covers_all_protos_with_seq():
    ...
    # Check that ALL commands with seq have overrides (if they have command types)
    for command in commands_with_seq:
        ...
        if command in visitor._COMMAND_TYPE_MAP:
            assert hasattr(visitor, method_name), (
                ...
                f"The command is in _COMMAND_TYPE_MAP but the override wasn't created"
            )
        else:
    ...
    # Check that ALL activation jobs with seq have overrides
    for job in activation_jobs_with_seq:
        ...
        if job in visitor._COMMAND_TYPE_MAP:
            assert hasattr(visitor, method_name), (
                ...
                f"The job is in _COMMAND_TYPE_MAP but the override wasn't created"
            )
        else:
